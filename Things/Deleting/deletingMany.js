...
async function Red(){
    try{
        ...
        const filter = {"name.first" : "Albert"};

        const U = await Call.deleteMany(filter, {"name.first" : "Isaac" , "name.last":"Newton"})
            const Document = await Call.findOne();             //Actually we can change any Multidata from the object;
