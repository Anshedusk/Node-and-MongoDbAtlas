...
async function Red(){
    try{
        ...
        const filter = {"name.last" : "Newton"};

        const U = await Call.updateOne(filter , {$set : {"name.first" : "Isaac"}}).then(async() => {
            const Document = await Call.findOne({"name.first" : "Isaac"});             //Actually we can change any data from the object;
